...
public class TurnBasedManager : MonoBehaviour
{
    ...
    IEnumerator EndBattleCoroutine()
    {
        ...
        CombatantData finalDefeatedNode = null; // 用于最终传递给CombatManager的数据
        ...
        else // LOST
        {
            ...
            {
                playerNodeInCombat = CombatManager.Instance.playerHeadData; // 如果救援已触发，战败的一定是玩家蛇头
            }

            // 被击败的玩家单位，是不是蛇头？
            if (playerNodeInCombat.Location == 0)
            {
                Debug.Log("玩家蛇头战败！全军覆没！");
                result = CombatResultType.PlayerAnnihilated;
                finalDefeatedNode = playerNodeInCombat; // 记录下战败的蛇头
            }
            else
            {
                // 只是损失了一个身体节点，不是Game Over

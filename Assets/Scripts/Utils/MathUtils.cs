...
public static class MathUtils
{
    ...
    /// <summary>
    /// 根据索引数组对数据数组进行重新排序。
    /// 保证数据数组的元素不会丢失或重复，只是位置被重新映射。
    /// </summary>
    /// <typeparam name="T">数据数组的任意类型。</typeparam>
    /// <param name="dataArray">要重新排序的数据数组。</param>
    /// <param name="indexMap">长度相同的索引数组，其元素值是 dataArray 的新索引。</param>
    /// <returns>重新排序后的新数组。</returns>
    /// <exception cref="ArgumentException">如果两个数组长度不一致或索引超出范围则抛出异常。</exception>
    public static T[] ReorderArrayByIndex<T>(T[] dataArray, int[] indexMap)
    {
        if (dataArray == null || indexMap == null || dataArray.Length != indexMap.Length)
        {
                return null;
        }
        int length = dataArray.Length;
        T[] reorderedArray = new T[length];

        for (int originalIndex = 0; originalIndex < length; originalIndex++)
        {
            int newIndex = indexMap[originalIndex];
            ...
            reorderedArray[newIndex] = dataArray[originalIndex];
        }
